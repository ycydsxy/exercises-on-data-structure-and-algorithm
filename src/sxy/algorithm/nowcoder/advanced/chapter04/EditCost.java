 ...
 * 最小编辑代价
 * 
 * 给定两个字符串str1和str2，再给定三个整数ic、dc和rc，分别代表插 入、删除和替换一个字符的代价，返回将str1编辑成str2的最小代价。
 * 
 * 经典动态规划，dp[i][j]表示从str1[0...i-1]变到str2[0...j-1]的最小代价（加入空字符串），则dp[i][j]=min(dp[
 * i-1][j]+rc,dp[i][j-1]+ic,str(i)==str(j)?dp[i-1][j-1]:dp[i-1][j-1]+rc)
 * 
